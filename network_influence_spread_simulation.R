...
V(graph)[random_index]$color = iteration_colors[1]
...
for(neighbor in neighbors(graph, V(graph)[random_index])) {
  ...
  V(graph)[neighbor]$color=iteration_colors[2]
}

jpeg("plot.jpg", width = 1280, height = 768)
...
legend(-1.15, .75, y.intersp = .75, xjust = .5, yjust = .5, legend=as.vector(legend_descriptions), bty = "n", fill = iteration_colors, cex=1.5)
dev.off()
